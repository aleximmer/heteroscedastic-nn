...
def gaussian_beta_log_likelihood_loss(mean, var, target, beta=1):
    ...
    return -(ll * weight).squeeze(1)

def gaussian_log_likelihood_loss(pred, target):
    mean =  pred[:,0].unsqueeze(1)
    var = pred[:,1].unsqueeze(1)
    ll = -0.5 * ((target - mean) ** 2 / var + torch.log(var) + _LOG_2PI)
    #print(-ll.squeeze(1).mean())
    #return -torch.sum(ll, axis=-1)
    return - ll.squeeze(1)

# Beta-NLL loss
def faithful_loss(mean, var, target):
    """Compute faithful loss

    :param mean: Predicted mean of shape B x D
    :param var: Predicted sigma of shape B x D
    :param target: Target of shape B x D
    :returns: Loss per batch element of shape B
    """
    loss = 0.5 * ((target - mean) ** 2) - dist.Normal(mean.detach(), torch.sqrt(var)).log_prob(target)
    return loss.sum(axis=-1)
...
class MLP_betanll(nn.Module):
    ...
    def forward(self, inp):
        ...
        return torch.cat(self.layers(inp), dim=-1)    # cat for compatibility with causa repo
...
class NormalGammaHead(nn.Module):
    def __init__(
        ...
        self.init_alpha_offset = softmax_inverse(initial_alpha - min_alpha)

        self.min_beta = min_beta
        self.max_beta = max_beta
        self.init_beta_offset = softmax_inverse(initial_beta - min_beta)
